      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 mb-8 border-b border-slate-800">
          ...
          {cartItems.length > 0 && (
            <div className="flex items-center gap-3">
              <Link
                to="/books"
                className="px-4 py-2 rounded-xl text-xs font-semibold text-slate-300 bg-slate-900 border border-slate-800 hover:bg-slate-800 transition"
              >
                + Add More Books
              </Link>
              <button
        ...
        {cartItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-900/40 border border-slate-800/80 rounded-3xl backdrop-blur-xl">
            <div className="w-24 h-24 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-600 mb-6">
              <ShoppingBag size={42} />
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              ...
              <Link
                to="/wishlist"
                className="px-6 py-3 bg-slate-900 border border-slate-800 hover:bg-slate-800 text-slate-300 font-semibold rounded-xl text-sm transition"
              >
                View Your Wishlist
              </Link>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 space-y-4">
              ...
              {summary && summary.warnings && summary.warnings.length > 0 && (
                <div className="p-4 rounded-2xl bg-amber-950/60 border border-amber-800/80 text-amber-200 text-xs space-y-1.5">
              ...
              {cartItems.map((item) => {
                ...
                return (
                  <div
                    key={item._id}
                    className="bg-slate-900/50 border border-slate-800/80 hover:border-slate-700/90 rounded-2xl p-4 sm:p-5 transition flex flex-col sm:flex-row items-center gap-4 sm:gap-6 group"
                  >
                    {/* Thumbnail */}
                    <Link
                      to={`/books/${item._id}`}
                      className="w-20 h-28 sm:w-24 sm:h-32 rounded-xl overflow-hidden bg-slate-950 flex-shrink-0 border border-slate-800 relative group-hover:border-indigo-500/40 transition"
                    >
                      <img
                        src={
                          item.image ||
                    ...
                    <div className="flex-1 min-w-0 text-center sm:text-left">
                      <Link
                        to={`/books/${item._id}`}
                        className="font-bold text-base sm:text-lg text-white hover:text-indigo-300 transition line-clamp-1"
                      >
                        {item.title}
                      </Link>
                      ...
                      <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-3">
                        {inStock ? (
                          itemStock < 5 ? (
                            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-400 bg-amber-950/60 border border-amber-800 px-2 py-0.5 rounded-md">
                          ...
                          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-rose-400 bg-rose-950/80 border border-rose-800 px-2 py-0.5 rounded-md">
                    ...
                    <div className="flex sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-4 sm:gap-3 pt-3 sm:pt-0 border-t sm:border-t-0 border-slate-800">
                      {/* Stepper */}
                      <div className="flex items-center border border-slate-700 bg-slate-950 rounded-xl p-1">
                        <button
                          ...
                          className="p-1.5 text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition rounded-lg hover:bg-slate-800"
                        >
                          <Minus size={13} />
                        </button>
            ...
            <div className="lg:col-span-4">
              <div className="bg-slate-900/70 border border-slate-800/80 p-6 rounded-3xl sticky top-24 backdrop-blur-xl shadow-2xl space-y-6">
                ...
                <div className="pt-4 border-t border-slate-800/80 space-y-2.5 text-slate-400 text-xs">
                ...
                <div className="pt-2 text-center">
                  ...
                  <div className="flex items-center justify-center gap-3">
                    <span className="px-2.5 py-1 bg-purple-950 text-purple-300 font-black text-[11px] rounded-lg border border-purple-800">
                      Khalti
                    </span>
                    <span className="px-2.5 py-1 bg-emerald-950 text-emerald-300 font-black text-[11px] rounded-lg border border-emerald-800">
                      eSewa
                    </span>
                    <span className="px-2.5 py-1 bg-slate-950 text-slate-300 font-bold text-[11px] rounded-lg border border-slate-800">
                      Cash on Delivery
                    </span>
                  </div>
                </div>
