...
export function AppShell() {
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 transition-colors duration-150">
      <nav className="bg-white/95 dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800 backdrop-blur-md sticky top-0 z-40 transition-colors duration-150 shadow-sm dark:shadow-none">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            >
              ...
              <span className="text-lg font-black tracking-tight text-slate-900 dark:text-white hidden sm:inline-block">
            ...
            <div className="hidden sm:flex items-center space-x-2.5">
              ...
              >
                <Heart size={19} />
                {wishlistCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-rose-600 text-white text-[10px] font-black px-1.5 py-0.2 rounded-full ring-2 ring-white dark:ring-slate-900 shadow">
                    {wishlistCount}
                  </span>
                )}
              </Link>
              ...
              >
                ...
                {cartCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-indigo-600 text-white text-[10px] font-black px-1.5 py-0.2 rounded-full ring-2 ring-white dark:ring-slate-900 shadow">
                    {cartCount}
                  </span>
                )}
              </Link>
                ...
                <MenuItems className="absolute right-0 mt-2 w-52 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-1.5 shadow-2xl z-50 divide-y divide-slate-100 dark:divide-slate-800">
                  <div className="p-1 space-y-0.5">
                    {isAdmin && (
                      <Link
                        to="/admin"
                        className="flex items-center gap-2 px-3 py-2 text-xs font-bold text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:bg-indigo-950/40 dark:hover:bg-indigo-900/60 rounded-xl transition border border-indigo-200 dark:border-indigo-800/60 mb-1"
                      >
                    ...
                    <Link
                      to="/orders"
                      className="flex items-center gap-2 px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition"
                    >
        ...
        {mobileMenuOpen && (
          <div className="sm:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 py-6 space-y-4">
            <SearchBar />
            ...
            <div className="space-y-1 pt-2">
              ...
              <Link
                to="/wishlist"
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800"
              >
