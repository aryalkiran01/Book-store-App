    ...
    <section aria-labelledby="recently-viewed-heading" className={`py-8 ${className}`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 id="recently-viewed-heading" className="text-lg font-bold text-slate-900">
            Recently Viewed
          </h2>
        </div>
        <button
          onClick={handleClear}
          className="text-xs text-slate-400 hover:text-red-600 transition-colors flex items-center gap-1 focus:outline-none focus:ring-2 focus:ring-red-500 rounded px-1"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {books.map((book) => (
          <Link
            key={book._id}
            to={`/books/${book._id}`}
            className="group block bg-white rounded-xl border border-slate-200/80 p-3 hover:shadow-md hover:border-indigo-200 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
            <div className="aspect-[3/4] bg-slate-100 rounded-lg overflow-hidden mb-2 relative">
              ...
              {book.discountPercentage ? (
                <span className="absolute top-1.5 left-1.5 bg-rose-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded shadow-sm">
                  -{book.discountPercentage}%
                </span>
              ) : null}
            </div>
            ...
            <p className="text-[11px] text-slate-500 truncate mb-1">{book.author}</p>

            <div className="flex items-center justify-between mt-auto">
              <span className="text-xs font-bold text-slate-900">${book.price.toFixed(2)}</span>
              {book.averageRating ? (
